...
struct hostent *host;
	...
	this	-> homeGrid	= 
	              settings -> value ("homeGrid", "JO21fx"). toString (). toStdString ();
	this	-> programName	= "Qt-FT8";
	this	-> antenna	= 
	              settings -> value ("antenna", "youloop"). toString (). toStdString ();
	settings	-> endGroup ();
	sequence		= 1;
	reporterOK		= false;
...
pskMessage m;
	...
	locker. unlock ();
	fprintf (stderr, "message added, stack has %d elements\n",
	                                 messageStack. size ());
}
...
int	reporterWriter::sendMessages	() {
	...
	fprintf (stderr, "sending %d starts\n", messageStack. size ());
...
//	size of seen transmitters records
	...
	fprintf (stderr, "Setting global data\n");
//	global data
	fprintf (stderr, "at %d we set data offset %d\n",
	                               offset_for_size, data_offset);
	(void)copy_int2 (&buffer [offset_for_size], data_offset);
	fprintf (stderr, "at %d we set the time\n",
	                        offset_for_transmission_time);
	(void)copy_int4 (&buffer [offset_for_transmission_time], time (nullptr));
	fprintf (stderr, "at %d we set the sequence number %d\n",
	                        offset_for_sequence_number, sequence);
	(void)copy_int4 (&buffer [offset_for_sequence_number], sequence);
	sequence ++;

	fprintf (stderr, "zenden begint\n");
	...
	locker. unlock ();
	fprintf (stderr, "Messages were sent\n");
	return EXIT_SUCCESS;
}
