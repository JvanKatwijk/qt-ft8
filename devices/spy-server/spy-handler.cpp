...
bool	spyHandler::show_attendance () {
...
std::vector<uint8_t> args =
	...
	memcpy (&args[0], protocolVersionBytes, sizeof(ProtocolVersion));
	memcpy (&args[0] + sizeof(ProtocolVersion),
	                         softwareVersionBytes, SoftwareID.size());
	bool res =  send_command (CMD_HELLO, args);
	return res;
}
	
void	spyHandler::cleanRecords () {
...
bool	spyHandler::send_command (uint32_t cmd, std::vector<uint8_t> &args) {
bool result;
...
bool	spyHandler::process_device_info (uint8_t *buffer,
	                                        DeviceInfo &deviceInfo) {
	memcpy (&deviceInfo, buffer, sizeof(DeviceInfo));
	std::cerr << "\n**********\nDevice Info:" 
...
bool	spyHandler::process_client_sync (uint8_t *buffer,
	                                         ClientSync &client_sync) {

	memcpy ((void *)(&client_sync), buffer, sizeof (ClientSync));
	memcpy ((void *)(&m_curr_client_sync), buffer, sizeof (ClientSync));
...
bool	spyHandler::get_deviceInfo (struct DeviceInfo &theDevice) {
	if (!got_device_info)
	   return false;
...
double	spyHandler::get_sample_rate	() {
	return 96000;
}
...
bool	spyHandler::set_gain		(double gain) {
std::vector<uint32_t> param (1);
...
bool	spyHandler::is_streaming	() {
	return streaming. load ();
}

void	spyHandler::start_running	() {
std::vector<uint32_t> p (1);
	if (!streaming. load ()) {
...
bool	spyHandler::set_setting (uint32_t settingType,
...
std::vector<uint8_t> argBytes;
	...
	if (params.size () > 0) {
	   ...
	   memcpy (&argBytes [0] + sizeof (uint32_t),
...
void	spyHandler::process_data (uint8_t *theBody, int length) {
	outB ->  putDataIntoBuffer (theBody, length);
	emit data_ready ();
}

void	spyHandler::connection_set () {
std::vector<uint32_t> p;
